        ...
        <div class="w-2/3 mx-auto">
            ...
            <div class="bg-white shadow-md rounded my-6">
                <table class="text-left w-full border-collapse">
                    ...
                    <tbody>
                    <tr class="hover:bg-grey-lighter" v-for="category in categories.data">
            ...
            <div class="flex justify-center">
                <ul class="flex list-reset border border-grey-light rounded w-auto font-sans">
                    <li v-if="categories.current_page > 1">
                        <a class="no-underline block hover:text-white hover:bg-blue text-blue border-r border-grey-light px-3 py-2"
                           :href="`/category?page=${categories.current_page - 1}&filter=${filterText}`">Previous</a>
                    </li>
                    <li v-for="(index) in categories.last_page">
                        <a :class="{
                        'bg-blue text-white' : categories.current_page === index,
                        'bg-white text-blue' : categories.current_page !== index,
                        'border-r' : index !== categories.last_page
                        }"
                    ...
                    <li v-if="categories.last_page > categories.current_page">
                        <a class="no-underline block hover:text-white hover:bg-blue text-blue px-3 py-2 border-l border-grey-light"
                           :href="`/category?page=${categories.current_page + 1}&filter=${filterText}`">Next</a>
                    </li>
                </ul>
            </div>
        </div>
...
<script>
    ...
    export default {
        props: ['categories', 'defaultFilter'],
